...
## Breve descripción 
Este proyecto tiene como objetivo implementar una pipeline Extract-Transform-Load (ETL) de parámetros climáticos de interés para la industria agricola. En principio toma valores recopilados de una API de referencia y calcula parámetros derivados de los mismos. El flujo de trabajo incluye una serie de validaciones para garantizar la calidad de los datos antes de almacenarlos en un storage cloud (AWS S3). Se prevé aumentar la cantidad  datos derivados calculados, pero para una mayor precisión se requiere la obtención de datos de fuentes más precisas, como sensores o estaciones metereologicas locales
